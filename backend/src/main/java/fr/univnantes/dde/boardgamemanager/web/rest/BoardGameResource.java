...
public class BoardGameResource {
    ...
    public ResponseEntity<BoardGame> updateBoardGame(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, boardGame.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<BoardGame> partialUpdateBoardGame(
        ...
        log.debug("REST request to partial update BoardGame partially : {}, {}", id, boardGame);
        if (boardGame.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, boardGame.getId())) {
        ...
        Optional<BoardGame> result = boardGameRepository
            ...
            .map(existingBoardGame -> {
                ...
                if (boardGame.getCoverContentType() != null) {
                    existingBoardGame.setCoverContentType(boardGame.getCoverContentType());
                }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, boardGame.getId().toString())
        );
    }
     ...
     */
    @GetMapping("")
    public List<BoardGame> getAllBoardGames(@RequestParam(required = false, defaultValue = "true") boolean eagerload) {
    ...
    public ResponseEntity<BoardGame> getBoardGame(@PathVariable Long id) {
        ...
        Optional<BoardGame> boardGame = boardGameRepository.findOneWithEagerRelationships(id);
