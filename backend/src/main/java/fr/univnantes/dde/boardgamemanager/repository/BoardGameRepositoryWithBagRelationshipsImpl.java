 ...
 */
public class BoardGameRepositoryWithBagRelationshipsImpl
  implements BoardGameRepositoryWithBagRelationships {

  @PersistenceContext
  private EntityManager entityManager;

  @Override
  public Optional<BoardGame> fetchBagRelationships(
  ...
  public Page<BoardGame> fetchBagRelationships(Page<BoardGame> boardGames) {
    return new PageImpl<>(
      fetchBagRelationships(boardGames.getContent()),
      boardGames.getPageable(),
  ...
  List<BoardGame> fetchPublishers(List<BoardGame> boardGames) {
    ...
    IntStream
      .range(0, boardGames.size())
      .forEach(index -> order.put(boardGames.get(index).getId(), index));
    ...
    Collections.sort(
      result,
      (o1, o2) -> Integer.compare(order.get(o1.getId()), order.get(o2.getId()))
    );
    return result;
  }
