...
export class SeriesService {
  ...
  update(series: ISeries): Observable<EntityResponseType> {
    return this.http.put<ISeries>(`${this.resourceUrl}/${this.getSeriesIdentifier(series)}`, series, { observe: 'response' });
  }

  partialUpdate(series: PartialUpdateSeries): Observable<EntityResponseType> {
    return this.http.patch<ISeries>(`${this.resourceUrl}/${this.getSeriesIdentifier(series)}`, series, { observe: 'response' });
  }
  ...
  compareSeries(o1: Pick<ISeries, 'id'> | null, o2: Pick<ISeries, 'id'> | null): boolean {
    return o1 && o2 ? this.getSeriesIdentifier(o1) === this.getSeriesIdentifier(o2) : o1 === o2;
  }
  ...
  addSeriesToCollectionIfMissing<Type extends Pick<ISeries, 'id'>>(
    ...
    if (series.length > 0) {
      const seriesCollectionIdentifiers = seriesCollection.map(seriesItem => this.getSeriesIdentifier(seriesItem)!);
