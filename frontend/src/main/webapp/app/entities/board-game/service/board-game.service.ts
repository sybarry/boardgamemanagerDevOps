...
export type PartialUpdateBoardGame = Partial<IBoardGame> &
  Pick<IBoardGame, "id">;
...
export class BoardGameService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/board-games");
  ...
  create(boardGame: NewBoardGame): Observable<EntityResponseType> {
    return this.http.post<IBoardGame>(this.resourceUrl, boardGame, {
      observe: "response",
    });
  }

  update(boardGame: IBoardGame): Observable<EntityResponseType> {
    return this.http.put<IBoardGame>(
      `${this.resourceUrl}/${this.getBoardGameIdentifier(boardGame)}`,
      boardGame,
      { observe: "response" },
    );
  }
  ...
  partialUpdate(
    boardGame: PartialUpdateBoardGame,
  ): Observable<EntityResponseType> {
    return this.http.patch<IBoardGame>(
      `${this.resourceUrl}/${this.getBoardGameIdentifier(boardGame)}`,
      boardGame,
      { observe: "response" },
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IBoardGame>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IBoardGame[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  compareBoardGame(
    o1: Pick<IBoardGame, "id"> | null,
    o2: Pick<IBoardGame, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getBoardGameIdentifier(o1) === this.getBoardGameIdentifier(o2)
      : o1 === o2;
  }
  ...
  addBoardGameToCollectionIfMissing<Type extends Pick<IBoardGame, "id">>(
    ...
    if (boardGames.length > 0) {
      const boardGameCollectionIdentifiers = boardGameCollection.map(
        (boardGameItem) => this.getBoardGameIdentifier(boardGameItem)!,
      );
