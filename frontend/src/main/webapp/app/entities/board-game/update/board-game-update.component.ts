...
import { BoardGameFormService, BoardGameFormGroup } from './board-game-form.service';
...
export class BoardGameUpdateComponent implements OnInit {
  ...
  editForm: BoardGameFormGroup = this.boardGameFormService.createBoardGameFormGroup();
  ...
  compareSeries = (o1: ISeries | null, o2: ISeries | null): boolean => this.seriesService.compareSeries(o1, o2);
    ...
    this.dataUtils.loadFileToForm(event, this.editForm, field, isImage).subscribe({
      error: (err: FileLoadError) =>
        this.eventManager.broadcast(
          new EventWithContent<AlertError>('boardgamemanagerApp.error', { ...err, key: 'error.file.' + err.key }),
        ),
    });
  }
  ...
  protected subscribeToSaveResponse(result: Observable<HttpResponse<IBoardGame>>): void {
  ...
  protected updateForm(boardGame: IBoardGame): void {
    ...
    this.seriesSharedCollection = this.seriesService.addSeriesToCollectionIfMissing<ISeries>(this.seriesSharedCollection, boardGame.series);
  ...
  protected loadRelationshipsOptions(): void {
    this.seriesService
      ...
      .pipe(map((series: ISeries[]) => this.seriesService.addSeriesToCollectionIfMissing<ISeries>(series, this.boardGame?.series)))
    ...
    this.publisherService
      ...
      .pipe(
        map((publishers: IPublisher[]) =>
          this.publisherService.addPublisherToCollectionIfMissing<IPublisher>(publishers, ...(this.boardGame?.publishers ?? [])),
        ),
      )
    ...
    this.categoryService
      ...
      .pipe(
        map((categories: ICategory[]) =>
          this.categoryService.addCategoryToCollectionIfMissing<ICategory>(categories, ...(this.boardGame?.categories ?? [])),
        ),
      )
