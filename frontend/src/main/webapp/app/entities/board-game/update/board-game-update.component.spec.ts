...
describe("BoardGame Management Update Component", () => {
  ...
  describe("ngOnInit", () => {
    it("Should call Series query and add missing value", () => {
      ...
      jest
        .spyOn(seriesService, "query")
        .mockReturnValue(of(new HttpResponse({ body: seriesCollection })));
      const additionalSeries = [series];
      const expectedCollection: ISeries[] = [
        ...additionalSeries,
        ...seriesCollection,
      ];
      jest
        .spyOn(seriesService, "addSeriesToCollectionIfMissing")
    ...
    it("Should call Publisher query and add missing value", () => {
      ...
      jest
        .spyOn(publisherService, "query")
        .mockReturnValue(of(new HttpResponse({ body: publisherCollection })));
      const additionalPublishers = [...publishers];
      const expectedCollection: IPublisher[] = [
        ...additionalPublishers,
        ...publisherCollection,
      ];
      jest
        .spyOn(publisherService, "addPublisherToCollectionIfMissing")
        .mockReturnValue(expectedCollection);
      ...
      expect(
        publisherService.addPublisherToCollectionIfMissing,
    ...
    it("Should call Category query and add missing value", () => {
      const boardGame: IBoardGame = { id: 456 };
      const categories: ICategory[] = [{ id: 19758 }];
      boardGame.categories = categories;

      const categoryCollection: ICategory[] = [{ id: 5828 }];
      jest
    ...
    it("Should update editForm", () => {
      ...
      boardGame.publishers = [publishers];
      const categories: ICategory = { id: 24849 };
      boardGame.categories = [categories];
  ...
  describe("save", () => {
    it("Should call update service on save for existing entity", () => {
      ...
      jest
        .spyOn(boardGameFormService, "getBoardGame")
        .mockReturnValue(boardGame);
      ...
      expect(boardGameService.update).toHaveBeenCalledWith(
        expect.objectContaining(boardGame),
      );
  ...
  describe("Compare relationships", () => {
    describe("compareSeries", () => {
      it("Should forward to seriesService", () => {
        ...
        expect(seriesService.compareSeries).toHaveBeenCalledWith(
          entity,
          entity2,
        );
      });
    ...
    describe("comparePublisher", () => {
      it("Should forward to publisherService", () => {
        ...
        expect(publisherService.comparePublisher).toHaveBeenCalledWith(
          entity,
          entity2,
        );
      });
    ...
    describe("compareCategory", () => {
      it("Should forward to categoryService", () => {
        ...
        expect(categoryService.compareCategory).toHaveBeenCalledWith(
          entity,
          entity2,
        );
      });
