...
export type PartialUpdatePublisher = Partial<IPublisher> &
  Pick<IPublisher, "id">;
...
export class PublisherService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/publishers");
  ...
  create(publisher: NewPublisher): Observable<EntityResponseType> {
    return this.http.post<IPublisher>(this.resourceUrl, publisher, {
      observe: "response",
    });
  }

  update(publisher: IPublisher): Observable<EntityResponseType> {
    return this.http.put<IPublisher>(
      `${this.resourceUrl}/${this.getPublisherIdentifier(publisher)}`,
      publisher,
      { observe: "response" },
    );
  }
  ...
  partialUpdate(
    publisher: PartialUpdatePublisher,
  ): Observable<EntityResponseType> {
    return this.http.patch<IPublisher>(
      `${this.resourceUrl}/${this.getPublisherIdentifier(publisher)}`,
      publisher,
      { observe: "response" },
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IPublisher>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IPublisher[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  comparePublisher(
    o1: Pick<IPublisher, "id"> | null,
    o2: Pick<IPublisher, "id"> | null,
  ): boolean {
    return o1 && o2
      ? this.getPublisherIdentifier(o1) === this.getPublisherIdentifier(o2)
      : o1 === o2;
  }
  ...
  addPublisherToCollectionIfMissing<Type extends Pick<IPublisher, "id">>(
    ...
    if (publishers.length > 0) {
      const publisherCollectionIdentifiers = publisherCollection.map(
        (publisherItem) => this.getPublisherIdentifier(publisherItem)!,
      );
